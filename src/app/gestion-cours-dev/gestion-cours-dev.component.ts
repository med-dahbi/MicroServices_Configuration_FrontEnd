...
@Component({
  selector: 'app-gestion-cours-dev',
  templateUrl: './gestion-cours-dev.component.html',
  styleUrls: ['./gestion-cours-dev.component.css']
})
export class GestionCoursDevComponent implements OnInit {
  ...
  constructor(private router:Router,private file :GestionCoursService,private accountService:AccountService) {
   ...
   update(){
    ...
    this.file.saveDeveloppement(this.PrincipalForm.value).subscribe(
  ...
  ngOnInit(): void {
    this.file.GetDevConfig().subscribe(data=>{
      console.log(data);
      this.data=data
